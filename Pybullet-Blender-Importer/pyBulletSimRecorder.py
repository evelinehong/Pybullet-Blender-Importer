...
class PyBulletRecorder:
    class LinkTracker:
        def __init__(self,
            ...
            self.ori_position = p.getLinkState(self.body_id,
                                            self.link_id,
                                            computeForwardKinematics=False)[4]
            self.diff = [self.link_pose[0][0] - self.position[0], self.link_pose[0][1] - self.position[1], self.link_pose[0][2] - self.position[2]]
